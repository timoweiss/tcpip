...
class TCP(object):
    ...
    # (evtl. sollte FIN intergriert werden)
    def gen_packet(self, seqn, ackn, syn, ack, payload):
        self.seqn = seqn
        self.ackn = ackn
        self.syn = syn
        self.ack = ack
    ...
    # Spezielle Funkton, um FIN Pakete zu erstellen
    def gen_fin(self, seqn, ackn, ack):
        self.seqn = seqn
        self.ackn = ackn
        self.ack = ack
        self.fin = 1
    ...
    # Test, ob FIN Paket (obsolet)
    def is_fin(self, packet):
        tcpp = self.unpack(packet)
        return tcpp.fin
